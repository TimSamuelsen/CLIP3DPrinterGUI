...
typedef void (*TL_MODULE_INITIALIZE)(void);

typedef void (*TL_MODULE_UNINITIALIZE)(void);
...
static void* thorlabs_unified_sdk_kernel_dll = 0;
#endif
...
#ifdef _WIN32
    if (thorlabs_unified_sdk_kernel_dll != NULL)
    {
        FreeLibrary(thorlabs_unified_sdk_kernel_dll);
    }
    thorlabs_unified_sdk_kernel_dll = NULL;
#endif

#ifdef __linux__
    if (thorlabs_unified_sdk_kernel_dll)
        dlclose(thorlabs_unified_sdk_kernel_dll);
    thorlabs_unified_sdk_kernel_dll = 0;
#endif
...
int tl_camera_sdk_dll_initialize(void)
{
...
#ifdef _WIN32
    thorlabs_unified_sdk_kernel_dll = LoadLibraryA("thorlabs_unified_sdk_kernel.dll");
    if (!thorlabs_unified_sdk_kernel_dll)
    {
    ...
    kernel_module_initialize = (TL_MODULE_INITIALIZE)GetProcAddress(thorlabs_unified_sdk_kernel_dll, (char*)"tl_module_initialize");
    if (!kernel_module_initialize)
    {
        return init_error_cleanup();
    }
    kernel_module_uninitialize = (TL_MODULE_UNINITIALIZE)GetProcAddress(thorlabs_unified_sdk_kernel_dll, (char*)"tl_module_uninitialize");
...
#ifdef __linux__
    thorlabs_unified_sdk_kernel_dll = dlopen("libthorlabs_unified_sdk_kernel.so", RTLD_LAZY);
    if (!thorlabs_unified_sdk_kernel_dll)
    {
        return (init_error_cleanup());
    }
    get_kernel_function = (TL_GET_FUNCTION)(dlsym(thorlabs_unified_sdk_kernel_dll, (char*)"tl_get_function"));
    if (!get_kernel_function)
    {
        return (init_error_cleanup());
    }
    kernel_module_initialize = (TL_MODULE_INITIALIZE)(dlsym(thorlabs_unified_sdk_kernel_dll, (char*)"tl_module_initialize"));
    if (!kernel_module_initialize)
    {
        return (init_error_cleanup());
    }
    kernel_module_uninitialize = (TL_MODULE_UNINITIALIZE)(dlsym(thorlabs_unified_sdk_kernel_dll, (char*)"tl_module_uninitialize"));
    if (!kernel_module_uninitialize)
    {
        return (init_error_cleanup());
    }
    ...
    tl_camera_get_user_memory = (TL_CAMERA_GET_USER_MEMORY)get_module_function(sdk_handle, "tl_camera_get_user_memory");
    if (!tl_camera_get_user_memory)
    {
        return init_error_cleanup();
    }

    tl_camera_get_user_memory_maximum_size = (TL_CAMERA_GET_USER_MEMORY_MAXIMUM_SIZE)get_module_function(sdk_handle, "tl_camera_get_user_memory_maximum_size");
    ...
    tl_camera_set_user_memory = (TL_CAMERA_SET_USER_MEMORY)get_module_function(sdk_handle, "tl_camera_set_user_memory");
